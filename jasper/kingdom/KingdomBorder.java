...
public class KingdomBorder {
		...
		//Calculized coordinates
		private double px = 0,py = 0,pz = 0; 	
		
		//More calc variables
		private double dist=0, angle=0, dt,df,pi2 = Math.PI * 2;
		
		//Index of which kingdom we've last calculated (-1 is neutral territory, -2 is undefined)
		private int prevKingdom = -2;	
		
		//More calc variables
		private int flag=0, section; 
        	...
        	if (event.entity instanceof EntityPlayer) 
        	{
        		...
        		if ( player.dimension == 0 ) {
	        		...
	        		if (this.count > 50 ) {
	    	        	...
	    	        	px = player.getPlayerCoordinates().posX;
	    	        	py = player.getPlayerCoordinates().posZ;
	    	    		...
	    	    		if ( flag > 0 ) { 
	    	    			...
	    	    			for(double r = 0; r < pi2; r += pi2 / 8) {
	    	    				...
	    	    				//absolute distance from spoke center
	    	    				df = Math.abs(df); 			
	    	    				
	    	    				//... in a spoke - assumption disproved
	    	    				if ( df < (flag == 1 ? 25 : 200) ) { 
	    	    					//inside a spoke-width (inner and outer spoke widths considered)
	    	    					found = false; 
	    	    					break;
	    	    				}
	    	    			} 
